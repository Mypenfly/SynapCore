...
impl AutoLoop {
    ...
    fn format_reflection_prompt(existing_reflection: &str) -> String {
        format!(
            "[System command]现在是AutoReflect模式。请你进行自我反思，总结关于用户画像、经验总结等内容。\n\n\
            请严格按照以下格式输出：\n\
            <reflection>\n\
            ## 用户画像 (User Profile)\n\
            - **基本信息**: [性别, 年龄, 职业等]\n\
            ...
            - **沟通效率**: [高效, 正常, 有待提高]\n\
            \n\
            ## 角色塑造或自我认知 (Role-building or self-awareness)\n\
            - **回答用语习惯**：[在交流中你总结出的用户偏好的你的用语习惯]\n\
            - **对角色的认知**: [你对你现在的身份的自我认知]\n\
            - **进一步塑造**: [你对你的角色的未来的塑造规划（基于用户）]\n\
            \n\
            ## 注意事项 (Notes)\n\
            1. 保持客观, 基于实际交互数据\n\
            2. 避免主观臆断\n\
    ...
    ///学习
    pub(crate) async fn auto_study(&mut self) -> AutoLoopResult<()> {
        ...
        学习过程建议使用web_search(查找有关资料),fetch_url(获取更详细内容),tools_manager(mode:search,获取有用的工具)。\
        工具调用轮数不要超过5\
    ...
    ///反思
    pub(crate) async fn auto_reflect(&mut self) -> AutoLoopResult<()> {
        let character = self.core.config.agent.leader.character.clone();
    ...
    ///自动清理
    pub(crate) async fn auto_clear(&mut self) -> AutoLoopResult<()> {
        let character = self.core.config.agent.leader.character.clone();
