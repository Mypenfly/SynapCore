...
fn shell(args: &Args) -> ToolResponse {
    ...
    if let Some(arguments) = args.args.as_ref() {
        for a in arguments {
            if a.contains("sudo") || a.contains("rm") {
                return ToolResponse::Error(format!(
                    "Function executor contains dangerouse command: {}",
                    choice
                ));
            }
    ...
    let output = match cmd.output() {
        ...
        Err(e) => {
            return ToolResponse::Error(format!(
                "Function executor failed in cmd {},error:{}",
                &program, e
            ));
        }
    };
    ...
    if text.is_empty() {
        ...
        return ToolResponse::Executor {
            command,
            content: error,
        };
    }

    ToolResponse::Executor {
        command,
        content: text,
    }
}
