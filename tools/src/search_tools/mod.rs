...
impl Tool for ToolsManager {
    fn definition(&self) -> crate::define_call::tool_define::ToolDefinition {
        ...
        let list: Vec<HashMap<String, String>> = self
            ...
            .iter()
            .map(|e| {
                let name = e.function.name.clone();
                let description = e.function.description.clone();
    ...
    async fn execute(
        ...
        let arguments = match &function.arguments {
            Some(a) => a,
            None => {
                return ToolResponse::Error("Function tools_manager lacks arguments".to_string());
            }
        };
        ...
        match args.action.as_str() {
            ...
            "add" => {
                ...
                if query == "all" {
                    ToolResponse::Manager {
                        mode: "add".to_string(),
                        definations: self.enabled.clone(),
                    }
                } else {
                    self.add(&query)
                }
            ...
            _ => ToolResponse::Error(format!(
                "Function tools_manager unkown action : {}",
                &args.action
            )),
        }
    }
...
impl ToolsManager {
    fn search(&self, query: &str) -> ToolResponse {
        let definations = self
            .enabled
            .iter()
            .filter(|e| e.function.name.contains(query))
            .cloned()
            .collect();
        ToolResponse::Manager {
