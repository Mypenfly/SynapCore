...
use crate::{
    define_call::{
        tool_call::Function,
        tool_define::{FunctionDefinition, Tool, ToolDefinition},
    },
    files_extract::extract_error::{ExtractErr, ExtractResult},
...
impl Tool for ExtractTool {
    fn definition(&self) -> crate::define_call::tool_define::ToolDefinition {
        ...
        let description = "文件内容提取，支持pdf,docx,xml,和各类代码文件或纯文本文件".to_string();
    ...
    async fn execute(&self, function: &Function) -> ToolResponse {
        let arguments = match &function.arguments {
            Some(s) => s,
            None => {
                return ToolResponse::Error("Function files_extract lacks arguments".to_string());
            }
        };
...
///不可解析文件
const SKIP_EXT: &[&str] = &[
    "exe", "dll", "so", "dylib", "bin", "mp3", "mp4", "wav", "avi", "mkv", "mox", "flac", "ogg",
    "zip", "tar", "gz", "rar", "7z", "bz2", "xz", "db", "sqlite", "sqlite3", "lock",
];

///二进制文件
const BINARY_EXT: &[&str] = &["pdf", "docx", "xlsx", "xls", "ods"];
...
fn extract_docx(path: &PathBuf) -> ExtractResult<String> {
    ...
    let file = std::fs::read(path).map_err(ExtractErr::File)?;
    let docx = read_docx(&file).map_err(ExtractErr::Docx)?;
...
fn extract_xml(path: &PathBuf) -> ExtractResult<String> {
    ...
    let mut reader = Reader::from_file(path).map_err(|e| ExtractErr::Xml(e.to_string()))?;
    ...
    loop {
        ...
        {
            Event::Text(e) => {
                let content = e.unescape().map_err(|e| ExtractErr::Xml(e.to_string()))?;
...
fn extract_text(path: &PathBuf) -> ExtractResult<String> {
    let bytes = std::fs::read(path).map_err(ExtractErr::File)?;
