...
**参数**:
- `message: &UserMessage` — 用户消息，包含文本、文件、是否启用工具、是否保存
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `text` | `String` | 用户的文本输入，会自动附加当前时间戳 |
...
## 八、Provider Crate 开发参考

### 8.1 当前状态

Provider crate 目前为空壳（仅有默认的 `add` 函数），其定位是 **Core的扩展和API初步封装**。

### 8.2 建议的封装方向

基于Core的现有API，Provider应提供以下能力：

#### 8.2.1 简化的初始化

```rust
// Provider应封装Core::init()并提供更友好的接口
pub struct SynapProvider {
    core: Core,
}

impl SynapProvider {
    pub fn new() -> Result<Self, CoreErr> {
        Ok(Self { core: Core::init()? })
    }
}
```
...
Core的 `task()/chat()` 返回 `mpsc::Receiver<BotResponse>`，Provider可封装为：

```rust
// 收集完整响应
pub async fn send_message(&mut self, text: &str) -> Result<String, CoreErr> {
    let message = UserMessage {
        text: text.to_string(),
        files: vec![],
        enable_tools: true,
    ...
    let mut rx = self.core.task(&message).await?;
    let mut full_response = String::new();
    while let Some(resp) = rx.recv().await {
        if let BotResponse::Content { chunk } = resp {
...
// 流式回调
pub async fn send_stream<F>(&mut self, text: &str, on_chunk: F) -> Result<(), CoreErr>
where F: Fn(BotResponse) {
    // ...
}
```
...
pub fn switch_leader(&mut self, character: &str) { ... }
```
...
pub fn update_config(&mut self, config: CoreConfig) -> Result<(), CoreErr> { ... }
```

#### 8.2.5 记忆管理

```rust
pub async fn search_memory(&self, query: &str) -> Result<Vec<SearchResult>, CoreErr> { ... }
pub async fn store_memory(&self, content: &str) -> Result<Memory, CoreErr> { ... }
```

### 8.3 关键注意事项

1. **Core不是Send/Safe的**: Core内部包含 `mpsc::Sender` 等类型，Provider需要注意线程安全
2. **事件循环是异步的**: `task()/chat()` 会spawn异步任务，Provider需要正确处理生命周期
3. **配置热重载**: Core的提示词支持热重载（`load_prompt`在每次创建Assistant时调用），Provider可利用此特性
4. **记忆系统依赖Embedding**: 记忆功能需要配置嵌入模型（`agent.embed`），否则 `store` 为 `None`
5. **工具系统独立**: Tools crate 是独立模块，Provider如需扩展工具应通过 `tools.toml` 的 `outer` 配置
...
```rust
use core::{Core, UserMessage, BotResponse};

pub async fn hello_synapcore() -> Result<(), Box<dyn std::error::Error>> {
    let mut core = Core::init()?;
